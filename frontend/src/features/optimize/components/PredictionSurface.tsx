...
export default function PredictionSurface({ campaignId, params, defaultX, defaultY }: Props) {
  ...
  return (<><ParamSelector params={params} xValue={xParam} yValue={yParam} onXChange={setXParam} onYChange={setYParam} />
    <PlotlyChart data={[
      { x: data.grid[0], y: data.grid[1], z: data.mean as number[][], type: 'contour', colorscale: 'RdYlGn', colorbar: { title: 'Score' },
        hovertemplate: `${xParam}: %{x:.1f}<br>${yParam}: %{y:.1f}<br>Score: %{z:.2f}<extra></extra>` },
      { x: data.measurements.map((m) => m.values[xParam]), y: data.measurements.map((m) => m.values[yParam]), mode: 'markers', type: 'scatter', name: 'Measurements',
