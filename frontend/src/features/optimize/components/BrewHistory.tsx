...
import { useBrews, type BrewListItem } from '@/features/brews/hooks';
...
export default function BrewHistory({ beanId, brewSetupId, effectiveRanges }: Props) {
  const { paginationModel, onPaginationModelChange, sortModel, onSortModelChange } = usePagination({ field: 'brewed_at', sort: 'desc' });
  ...
  if (!isLoading && items.length === 0)
    return <Box sx={{ p: 2, textAlign: 'center' }}><Typography color="text.secondary">No brews yet</Typography></Box>;
  ...
  const columns: GridColDef[] = [
    { field: 'index', headerName: '#', width: 60,
      renderCell: (params) => {
        ...
        return total - (page * size + params.api.getAllRowIds().indexOf(params.id));
      }, sortable: false },
    ...paramCols,
    { field: 'score', headerName: 'Score', width: 100,
      valueFormatter: (value: number | null) => value != null ? value.toFixed(1) : '—' },
    { field: 'is_failed', headerName: 'Status', width: 100,
      renderCell: (params) => {
        const failed = params.value as boolean;
        return <Chip label={failed ? 'failed' : 'ok'} size="small" color={failed ? 'error' : 'success'} />;
      } },
    { field: 'brewed_at', headerName: 'Brewed', width: 160,
      valueFormatter: (value: string) => new Date(value).toLocaleDateString() },
  ];

  return <DataTable columns={columns} rows={items} total={total} loading={isLoading}
